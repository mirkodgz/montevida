...
const BlogItem = ({ blog }: { blog: BlogItem }) => {
  return (
    <div className="shadow-1 bg-white rounded-xl px-4 sm:px-5 pt-5 pb-4">
      <Link href={`/blogs/${blog.slug || 'blog-details'}`} className="rounded-md overflow-hidden">
        <Image
          src={blog.img}
          alt="blog"
      ...
      <div className="mt-5.5">
        ...
        <h2 className="font-medium text-dark text-lg sm:text-xl ease-out duration-200 mb-4 hover:text-blue">
          <Link href={`/blogs/${blog.slug || 'blog-details'}`}>{blog.title}</Link>
        </h2>

        {blog.excerpt && <p className="text-dark-4 mb-4 line-clamp-3">{blog.excerpt}</p>}

        <Link
          href={`/blogs/${blog.slug || 'blog-details'}`}
