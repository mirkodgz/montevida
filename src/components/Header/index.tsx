      ...
      <div className="max-w-[1170px] mx-auto px-4 sm:px-7.5 xl:px-0">
        ...
        >
          ...
          <Link className="flex flex-row items-center gap-2 md:gap-3 flex-shrink-0" href="/">
            <div className="relative w-[45px] h-[45px] md:w-[55px] md:h-[55px] block">
              <Image
                src="/images/logo/LogoMonteVida-png.webp"
                alt="Logo Montevida"
                fill
                quality={100}
                priority
            ...
            <span
              className={`${forum.className} text-2xl md:text-3xl text-dark font-bold tracking-wide whitespace-nowrap mt-1`}
            >
              MONTEVIDA
            </span>
          </Link>
              ...
              <ul className="flex items-center gap-5 xl:gap-8">
                {menuData.map((menuItem, i) =>
                  menuItem.submenu ? (
                    ...
                    <li
                      key={i}
                      className="group relative before:w-0 before:h-[3px] before:bg-blue before:absolute before:left-0 before:-bottom-1 before:rounded-b-[3px] before:ease-out before:duration-200 hover:before:w-full"
                    >
                      <Link
                        href={menuItem.path}
          ...
          <div className="flex items-center gap-4 sm:gap-6">
            <div className="hidden sm:flex items-center gap-4">
              <Link href="/favoritos" className="flex items-center gap-1.5 font-medium text-custom-sm text-dark hover:text-blue">
                 <svg
            ...
            <Link href="/iniciar-sesion" className="flex items-center gap-1.5 font-medium text-custom-sm text-dark hover:text-blue">
              <svg
                width="20"
                height="20"
                viewBox="0 0 24 24"
            ...
            >
              <span className="inline-block relative">
                ...
                <span className="flex items-center justify-center font-medium text-[10px] absolute -right-1.5 -top-1.5 bg-blue w-4 h-4 rounded-full text-white">
                  {product.length}
                </span>
              </span>
              <span className="font-medium text-custom-sm text-dark hidden sm:inline-block pl-0.5">
        ...
        <div
          className={`w-full absolute left-0 top-full xl:hidden bg-white shadow-lg border-t border-gray-3 overflow-hidden transition-all duration-300 ease-in-out ${
            navigationOpen ? "max-h-[400px] visible py-4" : "max-h-0 invisible py-0"
          }`}
        >
          <nav className="px-4 sm:px-7.5">
            <ul className="flex flex-col gap-4">
              ...
              {/* Add fast links to mobile menu since header space is small */}
              <li className="pt-2 border-t border-gray-2 mt-2">
                 <Link href="/favoritos" onClick={() => setNavigationOpen(false)} className="flex items-center gap-2 text-custom-sm font-medium text-dark hover:text-blue">
                   Favoritos
                 </Link>
              </li>
            </ul>
